...
class ProductScout {
  ...
  async getManualProductList(category) {
    // ECHTE AFFILIATE-PRODUKTE vom User bereitgestellt
    // Lade aus config/real-affiliate-products.js

    const { REAL_AFFILIATE_PRODUCTS } = require('../../config/real-affiliate-products');

    // Filtere nach Kategorie
    const filteredProducts = REAL_AFFILIATE_PRODUCTS.filter(p => {
      // Normalisiere Kategorien für Matching
      const normalizedCategory = category.toLowerCase().replace(/-/g, '');
      const normalizedProductCategory = p.category.toLowerCase().replace(/-/g, '');
